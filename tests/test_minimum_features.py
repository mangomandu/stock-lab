"""
Find minimum feature set.

After ablation found MA & Momentum are redundant, test combinations
removing both. Find the smallest set with maximum alpha.

Output: results/minimum_features.txt
"""
import sys
import os
...
def main():
    ...
    w(f"[{datetime.now()}] Find minimum feature set")
    w("=" * 100)
    ...
    # Configs to test (focusing on removing redundant features)
    all_features = ['momentum', 'lowvol', 'trend', 'rsi', 'ma', 'volsurge']
    configs = [
        ('Baseline (6)',            list(all_features)),
        # Remove redundant single
        ('Without MA (5)',          ['momentum', 'lowvol', 'trend', 'rsi', 'volsurge']),
        ('Without Momentum (5)',    ['lowvol', 'trend', 'rsi', 'ma', 'volsurge']),
        # Remove both redundant
        ('Without MA + Mom (4)',    ['lowvol', 'trend', 'rsi', 'volsurge']),
        # Remove also weak trend
        ('Without MA+Mom+Trend (3)',['lowvol', 'rsi', 'volsurge']),
        # Just core 3
        ('Just lowvol+rsi+vol (3)', ['lowvol', 'rsi', 'volsurge']),
        # Add momentum back
        ('+Mom (lowvol+rsi+vol+mom)', ['momentum', 'lowvol', 'rsi', 'volsurge']),
        # Compare lowvol+momentum only (top 2 single)
        ('Just lowvol+momentum (2)',['momentum', 'lowvol']),
    ]
    ...
    w(f"{'Config':<30} {'#feat':>6} {'CAGR':>8} {'Sh':>5} {'MDD':>8} {'vs SPY':>9} {'t':>6}")
    w("-" * 100)
    for r in results:
        w(f"{r['label']:<30} {r['n_features']:>6} {r['avg_cagr']*100:>+7.2f}% {r['avg_sharpe']:>5.2f} "
          f"{r['avg_mdd']*100:>7.2f}% {r['avg_excess']:>+8.2f}%p {r['t_stat']:>6.2f}")

    out_path = os.path.join(OUTPUT_DIR, 'minimum_features.txt')
