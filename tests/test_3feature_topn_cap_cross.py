"""
Cross validation: Top-N × Sector Cap matrix on v5 (3-feature) model.

Top-N ∈ {10, 15, 20} × Cap ∈ {None, 30%, 25%, 20%, 15%}
= 15 configs × 31 windows.

v5 environment (S&P 500 + Ridge + 7y train + Weekly + 3 features).

Output: results/topn_cap_cross_3feature.txt
"""
import sys
import os
...
FEATURES = ['lowvol', 'rsi', 'volsurge']  # v5 minimum
...
def run_one_window(close, vol, test_year, hp, spy_ret, sectors, top_n, sector_cap):
    ...
    X_train_s = scaler.fit_transform(train_long[FEATURES].values)
    model = Ridge(alpha=1.0)
    model.fit(X_train_s, train_long['target'].values)
    X_test_s = scaler.transform(test_long[FEATURES].values)
...
def main():
    ...
    w(f"[{datetime.now()}] Top-N × Cap cross validation (v5 / 3-feature)")
    ...
    # Build matrix
    w(f"\n{'='*120}")
    w(f"## Sharpe matrix (Top-N × Cap) — v5 / 3-feature")
    ...
    out_path = os.path.join(OUTPUT_DIR, 'topn_cap_cross_3feature.txt')
